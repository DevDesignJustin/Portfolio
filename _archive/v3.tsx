...
function MagneticButton({
  href, className, children, onClick, dataCursor,
}: {
  ...
  return (
    <a ref={ref} href={href} className={className}
      ...
      onClick={onClick} data-cursor={dataCursor}>
      {children}
    </a>
  );
}
...
export default function V3() {
  ...
  // Marquee pause
  const pauseMarquee  = () => [marquee1, marquee2].forEach(r => { if (r.current) r.current.style.animationPlayState = "paused"; });
  const resumeMarquee = () => [marquee1, marquee2].forEach(r => { if (r.current) r.current.style.animationPlayState = "running"; });
  useGSAP(() => {
    // Progress bar
    ...
    gsap.from([justin.current, kim.current], {
      y: "110%", duration: 1, stagger: 0.08, ease: "power4.out", delay: 0.05,
    });
    gsap.from(".hero-sub", {
      y: 22, opacity: 0, duration: 0.75, stagger: 0.1, ease: "power3.out", delay: 0.75,
    });
    // Reveals
    ...
    gsap.from(".timeline-line", {
      ...
      scrollTrigger: { trigger: "#experience", start: "top 65%", end: "bottom 75%", scrub: 1 },
    });
  ...
  return (
    <div ref={containerRef} style={{ background: C.bg, color: C.text }} className="min-h-screen">
      ...
      <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-sm border-b"
        ...
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center justify-between">
          <span className="font-black text-sm tracking-tight" style={{ color: C.text }}>Justin Kim</span>
          <nav className="hidden md:flex items-center gap-7 text-sm">
            {NAV_SECTIONS.map((s) => {
              const isActive = activeSection === s;
              return (
                <a key={s} href={`#${s}`}
                  className="relative capitalize transition-colors flex items-center gap-1.5"
                  style={{ color: isActive ? C.accent : C.muted }}>
                  <span className="w-1 h-1 rounded-full transition-all duration-300"
                    style={{ background: C.accent, opacity: isActive ? 1 : 0, transform: isActive ? "scale(1)" : "scale(0)" }} />
                  {s}
                </a>
              );
            })}
            <Link href="/v2" className="font-semibold transition-colors" style={{ color: C.text }}>
              ← V2
            </Link>
            <Link href="/v4" className="font-semibold transition-colors" style={{ color: C.accent }}>
              V4 →
            </Link>
          </nav>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6">
        ...
        <section className="pt-32 pb-24 min-h-[90vh] flex flex-col justify-center">
          ...
          <div className="mt-8 flex items-center gap-6 flex-wrap">
            <p ref={roleRef}
              className="hero-sub font-mono text-[11px] tracking-[0.2em] uppercase"
              style={{ color: C.muted }}>
              {roles[roleIdx]}
            </p>
            <span className="hero-sub inline-flex items-center gap-1.5 font-mono text-[11px] tracking-widest uppercase"
              style={{ color: C.muted }}>
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: C.accent }} />
              Available
            </span>
          </div>
          <p className="hero-sub mt-6 text-base max-w-md leading-relaxed" style={{ color: C.muted }}>
          ...
          <div className="hero-sub mt-8 flex items-center gap-3">
            <MagneticButton href="#projects" dataCursor="EXPLORE"
              className="group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold rounded-full transition-all"
              style={{ background: C.accent, color: C.bg } as React.CSSProperties}>
              View Work
              <span className="transition-transform group-hover:translate-x-0.5">→</span>
            </MagneticButton>
            <MagneticButton href="#contact" dataCursor="HELLO ↗"
              className="group inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-full border transition-all hover:opacity-80"
              style={{ borderColor: C.border, color: C.text } as React.CSSProperties}>
              Get in Touch
              <span className="transition-transform group-hover:translate-x-0.5">↗</span>
            </MagneticButton>
          </div>
        ...
        <section id="about" className="py-24">
          <p className="reveal font-mono text-[11px] tracking-widest uppercase mb-12"
            style={{ color: C.accent }}>
            02 — About
          </p>
          <div className="reveal grid grid-cols-3 gap-6 mb-14 pb-14" style={{ borderBottom: `1px solid ${C.border}` }}>
            {[
              ...
              <div key={label}>
                <p className="font-black text-5xl md:text-6xl tracking-tight mb-1" style={{ color: C.text }}>
                  <Counter end={end} suffix={suffix} />
                </p>
                <p className="font-mono text-[11px] tracking-widest uppercase" style={{ color: C.subtle }}>
                  {label}
                </p>
              </div>
            ))}
            ...
            <div>
              {[
                ...
                <div key={label} className="flex justify-between items-center py-3.5"
                  ...
                  <span className="font-mono text-[11px] tracking-widest uppercase" style={{ color: C.subtle }}>
                    {label}
                  </span>
                  <span className="text-sm" style={{ color: C.muted }}>{val}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="projects" className="py-24">
          ...
          <div onMouseLeave={() => setHoveredProject(null)}>
            {projects.map((p, i) => (
              <a key={p.num} href="#" data-cursor="VIEW →"
                className="proj-item group flex gap-0 items-stretch py-8 -mx-6 px-6 transition-all duration-200"
                style={{
                  borderBottom:     `1px solid ${C.border}`,
                  opacity:          hoveredProject !== null && hoveredProject !== i ? 0.2 : 1,
                  backgroundColor:  hoveredProject === i ? C.surface : "transparent",
                }}
                onMouseEnter={() => setHoveredProject(i)}>
                {/* Big number */}
                <span className="font-black text-[4.5rem] leading-none select-none w-28 shrink-0 self-start mt-1 tabular-nums transition-colors"
                  style={{ color: hoveredProject === i ? C.accentDim : C.borderSoft }}>
                  {p.num}
                </span>
                <div className="flex-1 pt-1">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <span className="font-mono text-[10px] tracking-widest uppercase block mb-1.5"
                    ...
                    <span className="font-mono text-xs shrink-0 ml-4" style={{ color: C.subtle }}>{p.year}</span>
                  </div>
                  <p className="text-sm leading-relaxed mb-4 max-w-xl" style={{ color: C.muted }}>{p.desc}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap gap-1.5">
                      {p.stack.map((t) => (
                        <span key={t} className="font-mono text-[10px] px-2 py-0.5 rounded-full transition-colors"
                          style={{ border: `1px solid ${C.border}`, color: C.subtle }}>
                          {t}
                        </span>
                      ))}
                    </div>
                    <span className="text-xs font-medium shrink-0 ml-4 flex items-center gap-1 transition-colors"
                      style={{ color: hoveredProject === i ? C.accent : C.subtle }}>
                      View
                      <span className="transition-transform group-hover:translate-x-0.5">→</span>
                    </span>
                  </div>
        ...
        <section id="skills" className="py-24"
          onMouseEnter={pauseMarquee} onMouseLeave={resumeMarquee}>
          <p className="reveal font-mono text-[11px] tracking-widest uppercase mb-12"
            style={{ color: C.accent }}>
            04 — Skills
          </p>
          <div className="relative left-1/2 -translate-x-1/2 w-screen overflow-hidden">
            <div ref={marquee1} className="flex mb-3"
              style={{ animation: "marquee-left 22s linear infinite" }}>
              {[...allSkills, ...allSkills].map((s, i) => (
                <span key={i}
                  className="shrink-0 font-mono text-sm px-4 py-1.5 mr-3 rounded-full transition-all"
            ...
            <div ref={marquee2} className="flex"
              ...
              {[...allSkills.slice().reverse(), ...allSkills.slice().reverse()].map((s, i) => (
                <span key={i}
                  ...
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.background = "transparent";
                    (e.currentTarget as HTMLElement).style.color = C.subtle;
                    (e.currentTarget as HTMLElement).style.borderColor = C.borderSoft;
                  }}>
                  {s}
                </span>
              ))}
          ...
          <p className="reveal mt-8 font-mono text-[11px] tracking-widest text-center uppercase"
            style={{ color: C.subtle }}>
            Hover to pause
          </p>
        </section>
        ...
        <section id="experience" className="py-24">
          ...
          <div className="flex gap-10">
            <div className="relative flex flex-col items-center pt-1.5">
              <div className="timeline-line absolute top-0 bottom-0 w-px" style={{ background: C.border }} />
            </div>
            <div className="flex-1">
              {experience.map((exp) => (
                <div key={exp.company} className="reveal relative flex gap-6 pb-12 last:pb-0 group">
                  <div className="timeline-dot absolute -left-[2.85rem] top-1 w-3 h-3 rounded-full border-2 shrink-0 group-hover:scale-125 transition-transform"
                    style={{ background: C.accent, borderColor: C.bg, boxShadow: `0 0 0 1px ${C.border}` }} />
                  <div className="flex-1">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1 mb-2">
                      <div>
                        <h3 className="font-black text-lg tracking-tight" style={{ color: C.text }}>{exp.role}</h3>
                        <p className="text-sm" style={{ color: C.muted }}>{exp.company}</p>
                      </div>
                      <span className="font-mono text-xs shrink-0" style={{ color: C.subtle }}>{exp.period}</span>
                    </div>
                    <p className="text-sm leading-relaxed max-w-xl" style={{ color: C.muted }}>{exp.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="contact" className="py-24">
          ...
          <div className="reveal">
            <h2 className="font-black text-[clamp(3.5rem,10vw,8rem)] leading-[0.88] tracking-tight mb-10">
              <span style={{ color: C.text }}>LET&apos;S</span><br />
              <span style={{ color: C.accent }}>WORK.</span>
            </h2>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pt-8"
              ...
              <div className="flex flex-col items-start md:items-end gap-4">
                <MagneticButton
                  ...
                  onClick={copyEmail}
                  className="group inline-flex items-center gap-2 px-6 py-3 text-sm font-bold rounded-full transition-all"
                  style={{ background: C.accent, color: C.bg } as React.CSSProperties}>
      ...
      <footer style={{ borderTop: `1px solid ${C.border}` }}>
        <div className="max-w-5xl mx-auto px-6 py-8 flex items-center justify-between">
          <span className="font-mono text-xs" style={{ color: C.subtle }}>© 2024 Justin Kim</span>
          <span className="font-mono text-xs" style={{ color: C.subtle }}>Built with Next.js</span>
        </div>
      </footer>
    </div>
  );
